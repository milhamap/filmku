    ...
    <div class="mx-auto max-w-screen hidden lg:block">
        ...
        <aside class="fixed z-50 w-[360px] bg-stream-dark">
            <div class="flex flex-col p-12 border-r border-softpur overflow-y-hidden h-screen">
                ...
                <div class="links flex flex-col mt-16 gap-2">
                    ...
                    <router-link :to="{name: 'Dashboard.Favorite'}" class="side-link active bg-sky-800 font-semibold text-white flex items-center w-full p-3 rounded-2xl gap-[10px]">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
        ...
        <div class="ml-[410px] pr-[50px] overflow-hidden">
            <div class="py-[70px] flex flex-col gap-[50px]">
                ...
                <!-- Featured -->
                <div>
                    <div class="font-semibold text-[22px] text-white mb-[18px]">My Favorite</div>
                    <div class="grid grid-cols-2 gap-5 xl:gap-12">
                        <div v-for="favorite in favorites" :key="favorite.id" class="col-span-1 relative overflow-hidden group cursor-pointer">
                            <img :src="'/images/uploads/' + favorite.films.image" class="object-cover rounded-[30px] w-full h-[320px]" alt="">
                            <div
                                ...
                                <div class="px-7 pb-7">
                                    <div class="font-medium text-xl text-white">{{favorite.films.title}}</div>
                                    <p class="mb-0 text-white opacity-60 text-base mt-[10px]">
                                        {{ new Date(favorite.films.release_date).toDateString().split(' ')[2] + ' ' + new Date(favorite.films.release_date).toDateString().split(' ')[1] + ', ' + new Date(favorite.films.release_date).toDateString().split(' ')[3] }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <!-- <router-link :to="{name: 'Dashboard.DetailFavorite', params: {id: favorite.films.random}}" class="inset-0 absolute z-50"></router-link> -->
                        </div>
                    </div>
                </div>
                <!-- /Featured -->
